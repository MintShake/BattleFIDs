...
export async function GET(req: NextRequest) {
  ...
  const scores = await sql`
    SELECT fid, card_type, normalized_score, computed_at
...
// POST /api/week/score/compute — admin trigger
export async function POST(req: NextRequest) {
  ...
  const teamCards = await sql`
    SELECT DISTINCT
      unnest(ARRAY[captain_fid, broadcaster_fid, publisher_fid, agitator_fid, networker_fid]) AS fid
    FROM weekly_teams
    WHERE week_id = ${weekId}
  `;

  const fids = [...new Set(teamCards.map(r => r.fid).filter(Boolean) as number[])];

  // Fetch card metadata for all active cards
  const cardMeta = fids.length > 0
    ? await sql`SELECT fid, card_type, rarity FROM cards WHERE fid = ANY(${fids}::int[])`
    : [];

  const results: { fid: number; type: CardType; rawScore: number; normalizedScore: number }[] = [];
  ...
  for (const card of cardMeta) {
    ...
    await sql`
      INSERT INTO weekly_card_scores (week_id, fid, card_type, raw_score, normalized_score, computed_at)
      VALUES (${weekId}, ${card.fid}, ${type}, ${raw}, ${normalized}, NOW())
      ON CONFLICT (week_id, fid) DO UPDATE SET
        raw_score        = EXCLUDED.raw_score,
        normalized_score = EXCLUDED.normalized_score,
        computed_at      = NOW()
    `;

    results.push({ fid: card.fid, type, rawScore: raw, normalizedScore: normalized });
  }
  ...
  const scoreMap = new Map(results.map(r => [r.fid, r.normalizedScore]));
  const cardRarity = new Map(cardMeta.map(c => [c.fid as number, c.rarity as RarityTier]));

  const teamScores: { id: string; score: number }[] = [];

  for (const team of teams) {
    const captainRarity = cardRarity.get(team.captain_fid) ?? 'Common';
    const slots = [
      { type: 'CAPTAIN'     as CardType, score: 0 },
      { type: 'BROADCASTER' as CardType, score: scoreMap.get(team.broadcaster_fid) ?? 0 },
      { type: 'PUBLISHER'   as CardType, score: scoreMap.get(team.publisher_fid)   ?? 0 },
      { type: 'AGITATOR'    as CardType, score: scoreMap.get(team.agitator_fid)    ?? 0 },
      { type: 'NETWORKER'   as CardType, score: scoreMap.get(team.networker_fid)   ?? 0 },
    ];
  ...
  for (const team of teams) {
    const slotFids = [team.captain_fid, team.broadcaster_fid, team.publisher_fid, team.agitator_fid, team.networker_fid].filter(Boolean) as number[];
    const won = winningTeams.has(team.id);
    for (const fid of slotFids) {
      if (won) {
        await sql`UPDATE cards SET wins   = wins   + 1 WHERE fid = ${fid}`;
      } else {
        await sql`UPDATE cards SET losses = losses + 1 WHERE fid = ${fid}`;
      }
    }
  }

  // ── Xplora credit prizes ──────────────────────────────────────────────────
  const XP_PER_USDC     = 100;
  const FREE_TOP3_BONUS = 25;
  ...
  return NextResponse.json({ ok: true, weekId, cardsScored: results.length, teamsRanked: teams.length, creditPool, prizePool, payouts });
}
